...
$(eval $(call validate-option,COMPILER,ido emcc))
...
#ifeq ($(COMPILER),gcc)
ifeq ($(COMPILER),emcc)
  CFLAGS += -mno-shared -mfix4300 -mabi=32 -mhard-float -mdivide-breaks -fno-stack-protector -fno-common -fno-zero-initialized-in-bss -fno-PIC -mno-abicalls -fno-strict-aliasing -fno-inline-functions -ffreestanding -fwrapv -Wall -Wextra
#  CFLAGS += -mno-shared -march=vr4300 -mfix4300 -mabi=32 -mhard-float -mdivide-breaks -fno-stack-protector -fno-common -fno-zero-initialized-in-bss -fno-PIC -mno-abicalls -fno-strict-aliasing -fno-inline-functions -ffreestanding -fwrapv -Wall -Wextra
else
  CFLAGS += -non_shared -Wab,-r4300_mul -Xcpluscomm -Xfullwarn -signed -32
endif

ASFLAGS     := -mabi=32 $(foreach i,$(INCLUDE_DIRS),-I$(i)) $(foreach d,$(DEFINES),--defsym $(d))
...
#  PLATFORM_LDFLAGS := $(SDLCONFIG_LDFLAGS) -lm -lpthread `pkg-config --libs libusb-1.0` -no-pie
  PLATFORM_CFLAGS  := $(SDLCONFIG_CFLAGS) -DTARGET_LINUX
  PLATFORM_LDFLAGS := $(SDLCONFIG_LDFLAGS) -lm -lpthread -no-pie
endif
ifeq ($(TARGET_MACOS),1)
  SDLCONFIG_LDFLAGS := $(shell sdl2-config --libs)
#  PLATFORM_CFLAGS  := $(SDLCONFIG_CFLAGS) -DTARGET_MACOS -DTIMER_ABSTIME=1 `pkg-config --cflags libusb-1.0`
#  PLATFORM_LDFLAGS := $(SDLCONFIG_LDFLAGS) -lm -lpthread `pkg-config --libs libusb-1.0` -no-pie
  PLATFORM_CFLAGS  := $(SDLCONFIG_CFLAGS) -DTARGET_MACOS -DTIMER_ABSTIME=1
  PLATFORM_LDFLAGS := $(SDLCONFIG_LDFLAGS) -lm -lpthread -no-pie
endif
ifeq ($(TARGET_WEB),1)
...
# ^Old
#
#  PLATFORM_LDFLAGS := -lm -lGL -no-pie -s TOTAL_MEMORY=64MB -g3 -s `pkg-config libusb-1.0 --libs --cflags` "EXTRA_EXPORTED_RUNTIME_METHODS=['callMain']"
  PLATFORM_LDFLAGS := -lm -lGL -lSDL2 -no-pie -s TOTAL_MEMORY=64MB -g3 -s "EXTRA_EXPORTED_RUNTIME_METHODS=['callMain']"
endif
...
WASM_SDL := -DRAPI_GL=1 -DWAPI_SDL2=1 -DAAPI_SDL2=1 -DCAPI_SDL2=1 -DHAVE_SDL2=1 -sUSE_SDL=2
...
CC_CHECK := emcc -fsyntax-only -fsigned-char -Wall -Wextra -Wno-format-security -D_LANGUAGE_C $(DEF_INC_CFLAGS) $(PLATFORM_CFLAGS) $(GFX_CFLAGS) -s USE_SDL=2 -s USE_SDL_IMAGE=2
CFLAGS := $(OPT_FLAGS) -D_LANGUAGE_C $(WASM_SDL) $(DEF_INC_CFLAGS) $(PLATFORM_CFLAGS) $(GFX_CFLAGS) -fno-strict-aliasing -fwrapv -s USE_SDL=2
# $(WASM_SDL) is the new shit
